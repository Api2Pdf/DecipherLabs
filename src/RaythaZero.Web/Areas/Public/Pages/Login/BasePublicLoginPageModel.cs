...
using RaythaZero.Application.Login;
using RaythaZero.Domain.ValueObjects;

namespace RaythaZero.Web.Areas.Public.Pages.Login;

[AllowAnonymous]
public class BasePublicLoginPageModel : BasePublicPageModel
{
    ...
    public LoginAuthenticationSchemeChoiceItemViewModel EmailAndPassword => HasLoginByEmailAndPassword ? AuthenticationSchemes.First(p => p.AuthenticationSchemeType == AuthenticationSchemeType.EmailAndPassword) : null;
    public LoginAuthenticationSchemeChoiceItemViewModel MagicLink => HasLoginByMagicLink ? AuthenticationSchemes.First(p => p.AuthenticationSchemeType == AuthenticationSchemeType.MagicLink) : null;
    public IEnumerable<LoginAuthenticationSchemeChoiceItemViewModel> SingleSignOns => HasLoginBySingleSignOn ? AuthenticationSchemes.Where(p => p.AuthenticationSchemeType == AuthenticationSchemeType.Jwt || p.AuthenticationSchemeType == AuthenticationSchemeType.Saml) : null;
    public IEnumerable<LoginAuthenticationSchemeChoiceItemViewModel> AuthenticationSchemes { get; set; } = new List<LoginAuthenticationSchemeChoiceItemViewModel>();
